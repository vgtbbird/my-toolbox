...
const TotalStatsModule = {
    ...
    getAllRecords() {
        ...
        // ===== ✨ 跑玉魄 =====
        const soulTask = Storage.get('soulTask');
        if (soulTask && soulTask.history) {
            for (let h of soulTask.history) {
                const payload = h.payload || {};
                const profit = parseFloat(payload.profit) || 0;
                const income = parseFloat(payload.totalIncome) || 0;
                const cost = parseFloat(payload.totalCost) || 0;
                const rate = 0.08;
                const rmb = profit * rate;
                const dateStr = h._createdAt ? h._createdAt.split('T')[0] : '未知日期';
                ...
                records.push({
                    ...
                    icon: '✨',
                    type: '跑玉魄',
                    detail: `${payload.ringCount || 0}环 | 收入${income.toFixed(1)}万 | 成本${cost.toFixed(1)}万`,
                    income: income,
                    cost: cost,
                    profit: profit,
                    rmb: rmb,
